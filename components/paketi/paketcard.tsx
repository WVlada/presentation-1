...
export default function PaketCard({ paket }: Paket) {
  return (
    <div className="flex flex-col justify-center text-footertextcolor w-full mt-10 self-start ">
      ...
      <div className={"flex flex-col bg-white rounded p-6 text-center min-h-[500px] w-full shadow relative transition-transform duration-300 hover:scale-105 " + styles.cardHover}>
...
{paket.perks.map((perk) => {
          return (
            <div
              className={
                "cursor-pointer relative z-50 self-center " + styles.cardfadefromleft
              }
              key={perk}
            >
        ...
        <div className={"border-b w-full mb-10 mt-10 "}></div>
        <div className={" justify-center "}>
        <button
            type="button"
            className="text-sm transition-colors duration-700 hover:bg-white hover:text-tertiary text-center text-white hover:shadow bg-tertiary py-3 px-7 rounded-full"
          >
            Kontakt
          </button>
        </div>
